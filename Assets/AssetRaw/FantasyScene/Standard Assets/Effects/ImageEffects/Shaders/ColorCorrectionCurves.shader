...
Shader "Hidden/ColorCorrectionCurves" {
	...
	float4 _CameraDepthTexture_ST;//屏幕深度图_ST(SamplerTexture)
	...
	v2f vert( appdata_img v ) 
	{
		...
		o.uv =  v.texcoord.xy;
		o.uv2 = TRANSFORM_TEX(v.texcoord, _CameraDepthTexture);//拿顶点的uv去和材质球的tiling和offset作运算,等价于v.texcoord.xy * _MainTex_ST.xy + _MainTex_ST.zw;
		
		#if UNITY_UV_STARTS_AT_TOP //从1到0时
	...
	half4 frag(v2f i) : SV_Target 
	{
		...
		half theDepth = SAMPLE_DEPTH_TEXTURE( _CameraDepthTexture, i.uv2) ; //修正深度，处理平台差异问题
		half zval = tex2D(_ZCurve, half2( Linear01Depth (theDepth), 0.5)); //z深度值。Linear01Depth：将0-f映射到0-1
		...
		color = half4( lerp(red+green+blue, depthRed+depthBlue+depthGreen, zval), color.a);//根据深度取颜色

		half lum = Luminance(color.rgb); //亮度
