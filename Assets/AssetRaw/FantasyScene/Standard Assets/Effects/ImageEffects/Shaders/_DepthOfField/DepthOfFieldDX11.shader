...
Shader "Hidden/Dof/DX11Dof" 
{
	...
	float4 _BokehParams; //散景参数 legend: dx11BokehScale, dx11BokehIntensity,dx11BokehThreshhold, internalBlurWidth
	float4 _MainTex_TexelSize;
	float3 _Screen;
	float _SpawnHeuristic; //产生效果值

	sampler2D_float _CameraDepthTexture;
	sampler2D _BlurredColor;  //色彩模糊
	sampler2D _MainTex;
...
Pass
{
	...
	v2f vert (appdata v)
	{
		...
		#if UNITY_UV_STARTS_AT_TOP  //抗锯齿处理
	...
	float4 frag (v2f i) : SV_Target
	{
		...
		[branch] //UNITY_BRANCH这个宏在下列平台中被定义为[branch]：
		...
		if (c.a * _BokehParams.w > 1 && cblurred.a > 0.1 && lumc > _BokehParams.z && abs(lumc-lumblurred) > _SpawnHeuristic)
		{
			...
			append.color.rgba = float4(c.rgb * saturate(c.a*4), c.a); //saturate 限制值在0-1
